...
import { useSession, useProfilesManaged, useCreateProfile, useValidateHandle, useLogin, SessionType, profileId } from "@lens-protocol/react-web";
...
const RadioGroup = ({ defaultValue, children }: { defaultValue: string; children: React.ReactNode }) => (
  <div className="space-y-2">{children}</div>
);

const RadioGroupItem = ({ value, id, name }: { value: string; id: string; name: string }) => (
  <input
    type="radio"
    value={value}
    id={id}
...
export function AuthPage({ onSuccess }: AuthPageProps): JSX.Element {
  ...
  const handleCreateProfile = async (e: React.FormEvent): Promise<void> => {
    ...
    try {
      ...
      setSuccess(`Profile created successfully: ${result.value.handle?.fullHandle}!`);
      setLocalName("");
  ...
  const handleLogin = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    try {
      ...
      setSuccess(`Logged in successfully as ${result.value?.handle?.fullHandle ?? result.value?.id}`);
      if (onSuccess) {
        onSuccess();
      }
    } catch (err) {
            ...
            <TabsContent value="login" className="space-y-4 pt-4">
              {profilesLoading ? (
                ...
                <div className="text-center py-4">
                  <p className="text-muted-foreground mb-4">No Lens Profiles found in this wallet.</p>
                  <Button 
                    variant="outline" 
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex space-x-2">
                    <Input
                      ...
                      required
                    />
                    <span className="flex items-center text-muted-foreground">.lens</span>
                  </div>
    ...
    return (
      <Card className="w-full max-w-md mx-auto">
        <CardHeader>
          ...
          <CardDescription>
            You are connected to Lens Protocol
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
